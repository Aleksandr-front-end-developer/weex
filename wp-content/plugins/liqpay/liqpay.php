...
function woocommerce_init() {
	...
	class WC_Gateway_Liqpay extends WC_Payment_Gateway {
		...
		public function init_form_fields() {
			...
			$this->form_fields = array(
				...
				'icon'          => array(
					...
					'type'        => 'text',
					'default'     => 'https://www.liqpay.ua/1510216632270449/static/img/images/logo.svg',
		...
		public function generate_form( $order_id ) {
			...
			$description = __( "Оплата за заказ -" );
			...
			$html = $this->cnb_form( array(
				...
				'description' => $description . ' ' . $order_number,
		...
		public function cnb_form( $params ) {
            ...
            <form method="POST" action="%s" accept-charset="utf-8">
                %s
                %s' . $button . '
				<input type="submit" id="submit_payment_form" />
            </form>
			<script>jQuery("input#submit_payment_form").trigger("click");</script>
            ',
				$this->_checkout_url,
	...
	#[\AllowDynamicProperties]
	class WC_Gateway_Liqpay_Prepayment extends WC_Gateway_Liqpay {
		public function __construct() {

			$this->id                 = 'liqpay_prepayment';
			$this->has_fields         = false;
			$this->method_title       = __( 'LiqPay Предоплата', 'woocommerce' );
			$this->method_description = __( 'Платежная система LiqPay Предоплата', 'woocommerce' );
			$this->init_form_fields();
			$this->init_settings();
			$this->public_key  = $this->get_option( 'public_key' );
			$this->private_key = $this->get_option( 'private_key' );
			...
			$this->amount_prepayment = $this->get_option( 'amount_prepayment' );
		...
		public function init_form_fields() {
			...
			$this->form_fields['amount_prepayment'] = array(
				'title' => __( 'Сумма предоплаты', 'woocommerce' ),
				'type'  => 'text',				
			);
		}
		...
		public function generate_form( $order_id ) {
			...
			$description = __( "Предоплата за заказ -" );
			if ( function_exists( 'pll_current_language' ) ) {
				if ( pll_current_language() == 'en' ) {
					$description = 'Prepayment for the order -';
				}
				if ( pll_current_language() == 'uk' ) {
			...
			$html = $this->cnb_form( array(
				'version'     => '3',
				'amount'      => esc_attr( $this->amount_prepayment ),
				'currency'    => esc_attr( $currency ),
				'description' => $description . ' ' . $order_number,
				'order_id'    => esc_attr( $order_id ),
				'result_url'  => $redirect_page_url,
		...
		function check_ipn_response() {
			...
			if ( $success ) {
				...
				if ( $status == 'success' || ( $status == 'sandbox' && $this->sandbox == 'yes' ) ) {
					$order->update_status( $this->status, __( 'Заказ оплачен (Предоплата получена)', 'woocommerce' ) );
					$order->add_order_note( __( 'Клиент внес предоплату', 'woocommerce' ) );
					$woocommerce->cart->empty_cart();
				} else {
					$order->update_status( 'failed', __( 'Предоплата не была получена', 'woocommerce' ) );
					wp_redirect( $order->get_cancel_order_url() );
					exit;
				}
			} else {
	...
	function wm_liqpay( $methods ) {
		...
		$methods[] = 'WC_Gateway_Liqpay_Prepayment';
