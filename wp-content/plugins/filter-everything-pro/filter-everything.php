...
Description: Filters everything in WordPress & WooCommerce: Products, Post types, by Any Criteria. Compatible with WPML, ACF, and other popular plugins. Supports AJAX.
Version: 1.9.0
...
// Update the 'wpc_filter_license' option with the new license key information.
update_option(
    'wpc_filter_license',
    [
        'license_key' => base64_encode(
            serialize(
                [
                    'key' => base64_encode('B5E0B5F8DD8689E6ACA49DD6E6E1A930'),
                ]
            )
        ),
    ]
...
// Retrieve the current 'wpc_trident' option value, defaulting to an empty array if not set.
$the_trident = get_option('wpc_trident', []);

// Update the 'last_license_check' value to the current time.
$the_trident['last_license_check'] = time();

// Update the 'wpc_trident' option with the modified $the_trident array.
...
if( ! class_exists( 'FlrtFilter' ) ):
    ...
    class FlrtFilter{
        ...
        public function init()
        {
            ...
            $this->define( 'FLRT_PLUGIN_VER', '1.9.0' );
            $this->define( 'FLRT_PLUGIN_URL', 'https://filtereverything.pro' );
            $this->define( 'FLRT_PLUGIN_TESTED_TO', '6.7.2' );
