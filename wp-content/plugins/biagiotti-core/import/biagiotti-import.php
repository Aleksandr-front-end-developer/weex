...
class BiagiottiCoreImport {
	...
	function __construct() {
		...
		$this->importURI       = defined( 'BIAGIOTTI_MIKADO_PROFILE_SLUG' ) ? 'http://export.' . BIAGIOTTI_MIKADO_PROFILE_SLUG . '-themes.com/' : '';
	...
	public function import_content( $file ) {
		ob_start();
		require_once( BIAGIOTTI_CORE_ABS_PATH . '/import/class.wordpress-importer.php' );
		$biagiotti_import = new WP_Import();
		set_time_limit( 0 );
		
		$biagiotti_import->fetch_attachments = $this->attachments;
		$returned_value                  = $biagiotti_import->import( $file );
		
		if ( is_wp_error( $returned_value ) ) {
			$this->message = esc_html__( 'An Error Occurred During Import', 'biagiotti-core' );
		} else {
			$this->message = esc_html__( 'Content imported successfully', 'biagiotti-core' );
		}
	...
	public function import_widgets( $file, $file2 ) {
		...
		foreach ( (array) $options['widgets'] as $biagiotti_widget_id => $biagiotti_widget_data ) {
			update_option( 'widget_' . $biagiotti_widget_id, $biagiotti_widget_data );
		}
		
		$this->import_sidebars_widgets( $file );
		$this->message = esc_html__( 'Widgets imported successfully', 'biagiotti-core' );
	}
	...
	public function import_custom_sidebars( $file ) {
		...
		$this->message = esc_html__( 'Custom sidebars imported successfully', 'biagiotti-core' );
	}
	
	public function import_options( $file ) {
		$options       = $this->file_options( $file );
		$result        = update_option( 'mkdf_options_biagiotti', $options );
		$this->message = esc_html__( 'Options imported successfully', 'biagiotti-core' );
	}
	
	public function import_menus( $file ) {
		global $wpdb;
		
		$menus_data  = $this->file_options( $file );
		$menu_array  = array();
	...
	public function import_settings_pages( $file ) {
		...
		foreach ( $pages as $biagiotti_page_option => $biagiotti_page_id ) {
			update_option( $biagiotti_page_option, $biagiotti_page_id );
		}
	}
	...
	function mkdf_admin_import() {
		if ( biagiotti_core_theme_installed() ) {
			...
			$this->pagehook = add_submenu_page(
				...
				esc_html__( 'Biagiotti Options - Biagiotti Import', 'biagiotti-core' ), // The value used to populate the browser's title bar when the menu page is active
				esc_html__( 'Import', 'biagiotti-core' ),                     // The text of the menu in the administrator's sidebar
				'administrator',                                          // What roles are able to access the menu
                BIAGIOTTI_MIKADO_OPTIONS_SLUG . $slug,                  // The ID used to bind submenu items to this menu
				array( $biagiotti_mikado_global_Framework->getSkin(), 'renderImport' )
			);
			
			add_action( 'admin_print_scripts-' . $this->pagehook, 'biagiotti_mikado_enqueue_admin_scripts' );
			add_action( 'admin_print_styles-' . $this->pagehook, 'biagiotti_mikado_enqueue_admin_styles' );
		}
	}
	...
	function mkdf_update_options_after_import( $folder ) {
		...
		foreach ( $demo_urls as $demo_url ) {
			$global_options    = get_option( 'mkdf_options_biagiotti' );
			$new_global_values = str_replace( $demo_url, $url, $global_options );
			
			update_option( 'mkdf_options_biagiotti', $new_global_values );
		}
	}
	
	function mkdf_import_get_demo_urls( $folder ) {
		$demo_urls  = array();
		$domain_url = defined( 'BIAGIOTTI_MIKADO_PROFILE_SLUG' ) ? str_replace( '/', '', $folder ) . '.' . BIAGIOTTI_MIKADO_PROFILE_SLUG . '-themes.com/' : '';
