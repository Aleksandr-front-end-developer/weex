...
class Team implements lib\ShortcodeInterface {
	...
	public function vcMap() {
		...
		for ( $x = 1; $x < 6; $x ++ ) {
			$teamIconCollections = biagiotti_mikado_icon_collections()->getCollectionsWithSocialIcons();
			foreach ( $teamIconCollections as $collection_key => $collection ) {
				
				$team_social_icons_array[] = array(
					'type'       => 'dropdown',
					'heading'    => esc_html__( 'Social Icon ', 'biagiotti-core' ) . $x,
			...
			$team_social_icons_array[] = array(
				'type'       => 'textfield',
				'heading'    => esc_html__( 'Social Icon ', 'biagiotti-core' ) . $x . esc_html__( ' Link', 'biagiotti-core' ),
				'param_name' => 'team_social_icon_' . $x . '_link',
				'dependency' => array(
					'element' => 'team_social_icon_pack',
					'value'   => biagiotti_mikado_icon_collections()->getIconCollectionsKeys()
				)
			);
			
			$team_social_icons_array[] = array(
				'type'       => 'dropdown',
				'heading'    => esc_html__( 'Social Icon ', 'biagiotti-core' ) . $x . esc_html__( ' Target', 'biagiotti-core' ),
				'param_name' => 'team_social_icon_' . $x . '_target',
				'value'      => array(
					esc_html__( 'Same Window', 'biagiotti-core' ) => '_self',
					esc_html__( 'New Window', 'biagiotti-core' )  => '_blank'
				),
		...
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Team', 'biagiotti-core' ),
					'base'                      => $this->base,
					...
					'params'                    => array_merge(
						array(
							array(
								...
								'heading'     => esc_html__( 'Orientation', 'biagiotti-core' ),
								'value'       => array(
							...
							array(
								...
								'heading'    => esc_html__( 'Name', 'biagiotti-core' )
							),
							array(
								'type'        => 'dropdown',
								'param_name'  => 'team_name_tag',
								'heading'     => esc_html__( 'Name Tag', 'biagiotti-core' ),
							...
							array(
								...
								'heading'    => esc_html__( 'Name Color', 'biagiotti-core' ),
								'dependency' => array( 'element' => 'team_name', 'not_empty' => true )
							),
							array(
								'type'       => 'textfield',
								'param_name' => 'team_position',
								'heading'    => esc_html__( 'Position', 'biagiotti-core' )
							),
							array(
								'type'       => 'colorpicker',
								'param_name' => 'team_position_color',
								'heading'    => esc_html__( 'Position Color', 'biagiotti-core' ),
							...
							array(
								...
								'heading'     => esc_html__( 'Social Icon Pack', 'biagiotti-core' ),
								'value'       => array_merge( array( '' => '' ), biagiotti_mikado_icon_collections()->getIconCollectionsVCExclude( 'linea_icons' ) ),
								'save_always' => true
							),
						),
	...
	public function render( $atts, $content = null ) {
		...
		for ( $x = 1; $x <= $number_of_social_icons; $x ++ ) {
			...
			foreach ( biagiotti_mikado_icon_collections()->iconCollections as $collection_key => $collection ) {
		...
		$params['orientation']          = ! empty( $params['orientation'] ) ? $params['orientation'] : $args['orientation'];
		...
		$html = biagiotti_core_get_shortcode_module_template_part( 'templates/team', 'team', '', $params );
	...
	private function getTeamSocialIcons( $params ) {
		...
		if ( $team_social_icon_pack !== '' ) {
			...
			$icon_pack                    = biagiotti_mikado_icon_collections()->getIconCollection( $team_social_icon_pack );
			...
			for ( $i = 1; $i <= $params['number_of_social_icons']; $i ++ ) {
				...
				if ( $team_social_icon !== '' ) {
					...
					$social_icons[] = biagiotti_mikado_execute_shortcode( 'mkdf_icon', $team_icon_params );
				}
			}
		}
