<?php if($filter == 'yes') {
	...
	?>
	<div class="mkdf-pl-filter-holder" <?php biagiotti_mikado_inline_style($filter_holder_styles); ?>>
		<div class="mkdf-plf-inner">
			<?php
			if(is_array($filter_categories) && count($filter_categories)){ ?>
				<ul <?php biagiotti_mikado_inline_style($filter_styles); ?>>
					<li class="mkdf-pl-filter" data-filter="">
						<span><?php esc_html_e('Show all', 'biagiotti-core')?></span>
					</li>
