<div class="mkdf-testimonials-holder clearfix <?php echo esc_attr( $holder_classes ); ?>">
	<div class="mkdf-testimonials mkdf-owl-custom-slider mkdf-testimonials-main" <?php echo biagiotti_mikado_get_inline_attrs( $data_attr ) ?>>
		
		<?php if ( $query_results->have_posts() ):
			while ( $query_results->have_posts() ) : $query_results->the_post();
				$tagline             = get_post_meta( get_the_ID(), 'mkdf_testimonial_tagline', true );
				...
				<div class="mkdf-testimonial-content" id="mkdf-testimonials-<?php echo esc_attr( $current_id ) ?>">
					<div class="mkdf-testimonial-text-holder">
						<?php if ( ! empty( $tagline ) ) { ?>
							<p itemprop="name"
							   class="mkdf-testimonial-tagline entry-title"><?php echo esc_html( $tagline ); ?></p>
						<?php } ?>
						<?php if ( ! empty( $title ) ) { ?>
							<h2 itemprop="name"
							    class="mkdf-testimonial-title entry-title"><?php echo esc_html( $title ); ?></h2>
						<?php } ?>
						<?php if ( ! empty( $text ) ) { ?>
			...
			echo esc_html__( 'Sorry, no posts matched your criteria.', 'biagiotti-core' );
		endif;
	...
	<div class="mkdf-testimonial-image-nav mkdf-owl-custom-slider">
		<?php foreach ( $pagination_images as $image ) { ?>
			<div class="mkdf-testimonial-image"><?php echo biagiotti_mikado_get_module_part( $image ); ?></div>
		<?php } ?>
	</div>
