...
class BiagiottiCoreImport {
	...
	public function set_import_url() {
		$params = BiagiottiCoreDashboard::get_instance()->get_import_params();
	...
	public function import_action() {
			...
			if (isset($_POST) || !empty($_POST) || isset($_POST['options']['demo'])) {
				...
				if ( wp_verify_nonce( $_POST['options']['nonce'], 'mkdf_cd_import_nonce' ) ) {
					...
					switch ($_POST['options']['action']):
						...
						case 'rev-slider':
							if (biagiotti_core_is_revolution_slider_installed()) {
								...
								$this->set_message(esc_html__('Revolution Slider isn\'t installed', 'biagiotti-core'));
							}
							break;
						case 'content':
							...
							if($content_start) {
								if (!BiagiottiCoreDashboard::get_instance()->check_purchase_code($_POST['options']['demo'])) {
									biagiotti_core_ajax_status('error', esc_html__('Please don\'t try to hack me. Purchase code registered is not valid', 'biagiotti-core'));
									exit;
								}
							}
			...
			biagiotti_core_ajax_status($this->get_status(), $this->get_message(), $this->get_data());
		}
		wp_die();
	}
	...
	function file_content( $path ) {
		...
		if ( 200 !== intval( $response_code ) ) {
			$this->set_message($response["response"]['message'] . ' ' . esc_html__('Please contact support', 'biagiotti-core'));
	...
	public function import_widgets($demo) {
		...
		if($widgets_content) {
			foreach ((array)$widgets_content['widgets'] as $biagiotti_widget_id => $biagiotti_widget_data) {
				update_option('widget_' . $biagiotti_widget_id, $biagiotti_widget_data);
			}
			$ws = $this->import_sidebars_widgets($widgets);
			if($ws) {
				$this->set_message(esc_html__('Widgets are set for proper sidebar', 'biagiotti-core'));
				$this->set_data('type', 'options');
				$this->set_status('success');
			}
		}
	...
	public function import_sidebars_widgets( $file ) {
		$biagiotti_sidebars = get_option( "sidebars_widgets" );
		unset( $biagiotti_sidebars['array_version'] );
		$data = $this->unserialized_content( $file );

		if ( $data && is_array( $data['sidebars'] ) ) {
			$biagiotti_sidebars = array_merge( (array) $biagiotti_sidebars, (array) $data['sidebars'] );
			unset( $biagiotti_sidebars['wp_inactive_widgets'] );
			$biagiotti_sidebars                  = array_merge( array( 'wp_inactive_widgets' => array() ), $biagiotti_sidebars );
			$biagiotti_sidebars['array_version'] = 2;
			wp_set_sidebars_widgets( $biagiotti_sidebars );
			return true;
		} else {
			return false;
		}
	...
	public function import_custom_sidebars( $file ) {
		...
		if($options) {
			$results = update_option('mkdf_sidebars', $options);
	...
	public function import_options( $file ) {
		...
		$current_options = get_option(BIAGIOTTI_CORE_OPTIONS_NAME);
		if($options){
			if($current_options != $options) {
				$result = update_option(BIAGIOTTI_CORE_OPTIONS_NAME, $options);
				if ($result) {
					$this->update_options_after_import($file);
					$this->set_status('success');
					$this->set_data('type', 'options');
					$this->set_message(esc_html__('Options imported successfully', 'biagiotti-core'));

					$this->update_options_after_import($file);

				} else {
					$this->set_status('error');
					$this->set_message(esc_html__('Problem occurred during options import', 'biagiotti-core'));
				}
			} else {
				$this->set_status('success');
				$this->set_data('type', 'options');
				$this->set_message(esc_html__('Options are already imported', 'biagiotti-core'));
			}
		}

		if(biagiotti_mikado_is_elementor_installed()){
	...
	public function import_settings_pages( $file ) {
		...
		$new_ids = get_transient( '_biagiotti_core_imported_posts' );
		$fields_status = true;
		...
		if($pages) {
			...
			if (!$fields_status) {
				$this->set_status('error');
				$this->set_message(esc_html__('Problem occurred during settings pages import', 'biagiotti-core'));
			} else {
				$this->set_status('success');
				$this->set_data('type', 'options');
				$this->set_message(esc_html__('Settings pages imported successfully', 'biagiotti-core'));
			}
		} else {
			$this->set_status('error');
			$this->set_message(esc_html__('File doesn\'t exist', 'biagiotti-core'));
		}
	}
	...
	public function import_menu_settings( $file ) {
		...
		if($menus_data) {
			...
			$this->set_status('success');
			$this->set_data('type', 'options');
			$this->set_message( esc_html__( 'Menus set for proper locations', 'biagiotti-core' ) );
		} else {
			$this->set_status('error');
			$this->set_message( esc_html__( 'Problem during menus location set', 'biagiotti-core' ) );
		}
	}
	...
	public function import_content( $file, $xml, $attachments, $post_id) {
		ob_start();
		require_once( BIAGIOTTI_CORE_ABS_PATH . '/core-dashboard/sub-pages/import/wordpress-importer.php' );
...
//		}
//
		if(biagiotti_core_is_woocommerce_installed()) {
		...
		$biagiotti_import = new WP_Import();
		set_time_limit( 0 );

		$biagiotti_import->fetch_attachments = $attachments;
		$returned_value                  = $biagiotti_import->import( $file . $xml );
		...
		if ( is_wp_error( $returned_value ) ) {
			...
			$this->set_message( esc_html__( 'An error occurred during content import', 'biagiotti-core' ) );
		} else {
			$this->set_status('success');
			$this->set_data('type', 'content');
			$this->set_data('posts', $this->imported_posts);
			$this->set_message( esc_html__( 'File imported successfully', 'biagiotti-core' ) . ' ' . $xml );
		...
		if($files_created) {
			...
			foreach ($rev_sliders as $rev_slider) {
				...
				if (!$rev_results['success']) {
					$this->set_status('error');
					$this->set_message(esc_html__('Error while importing rev sliders', 'biagiotti-core'));
					exit;
				}
			}
			...
			$this->set_status('success');
			$this->set_data('type', 'options');
			$this->set_message(esc_html__('Rev sliders imported successfully', 'biagiotti-core'));
		} else {
			$this->set_status('error');
			$this->set_data('type', 'options');
			$this->set_message(esc_html__('Files don\'t exist', 'biagiotti-core'));
		}
	}
	...
	function update_options_after_import( $folder ) {
		...
		foreach ( $demo_urls as $demo_url ) {
			$global_options    = get_option( BIAGIOTTI_CORE_OPTIONS_NAME );
			$new_global_values = str_replace( $demo_url, $url, $global_options );

			update_option( BIAGIOTTI_CORE_OPTIONS_NAME, $new_global_values );
		}
	}
	...
	function import_get_demo_urls( $folder ) {
		...
		$domain_url = defined( 'BIAGIOTTI_MIKADO_PROFILE_SLUG' ) ? str_replace( '/', '', $folder ) . '.qodeinteractive.com/' : '';
	...
	function get_noimage_url( $origin_img_url ) {
		...
		return BIAGIOTTI_CORE_ASSETS_URL_PATH . '/img/' . $noimage_fname;
	}
	...
	public function populate_single_pages() {
		...
		if ( isset( $_POST ) && !empty( $_POST ) && !empty($_POST['options']['demo']) ) {
			if ( wp_verify_nonce( $_POST['options']['nonce'], 'mkdf_cd_import_nonce' ) ) {
				...
				$html = biagiotti_core_get_module_template_part('core-dashboard/sub-pages/import', 'pages-list', '', array('pages' => $pages));

				if($pages){
					biagiotti_core_ajax_status( 'success', '', $html);
				} else {
					biagiotti_core_ajax_status( 'error', esc_html__( 'Pages don\'t exist', 'biagiotti-core' ), '');
				}
			}
		}
	...
	public function is_ready_to_import() {
		$info = BiagiottiCoreSystemInfoPage::get_instance()->get_system_info();
...
BiagiottiCoreImport::get_instance();
