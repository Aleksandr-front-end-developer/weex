<?php if ( is_array( $post_ratings ) && count( $post_ratings ) ) { ?>
	<?php $average_rating_total = biagiotti_core_get_total_average_rating( $post_ratings ); ?>
				...
				<div class="mkdf-grid-col-3">
					<div class="mkdf-item-reviews-average-wrapper">
						<div class="mkdf-item-reviews-average-rating">
							<?php echo esc_html( biagiotti_core_reviews_format_rating_output( $average_rating_total ) ); ?>
						</div>
						<div class="mkdf-item-reviews-verbal-description">
                            <span class="mkdf-item-reviews-rating-icon">
                                <?php echo biagiotti_core_reviews_get_icon_for_rating( $average_rating_total ); ?>
                            </span>
							<span class="mkdf-item-reviews-rating-description">
                                <?php echo esc_html( biagiotti_core_reviews_get_description_for_rating( $average_rating_total ) ); ?>
                            </span>
						</div>
					</div>
				</div>
				<div class="mkdf-grid-col-9">
					<div class="mkdf-rating-percentage-wrapper">
						...
						foreach ( $post_ratings as $rating ) {
							$percentage = biagiotti_core_post_average_rating_per_criteria( $rating );
							echo do_shortcode( '[mkdf_progress_bar percent="' . esc_attr( $percentage ) . '" title="' . esc_attr( $rating['label'] ) . '"]' );
						}
						?>
					</div>
				</div>
