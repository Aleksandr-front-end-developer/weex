...
function woocommerce_wayforpay_init()
{
    ...
    class WC_wayforpay extends WC_Payment_Gateway
    {
        ...
        public function __construct()
        {
			...
			$array_explode     = explode( '::', $this->settings['description'] );
	...
	<script>
        ...
        function generate_wayforpay_form($order_id)
        {
            ...
            $wayforpay_args = array(
                ...
                'returnUrl' => $order->get_checkout_order_received_url() . '&order='.$order_id .'&redirect_to_checkout=1', //$this->getCallbackUrl().'?key='.$order->order_key.'&order='.$order_id,
...
add_action('template_redirect', function(){
	if ( isset( $_GET['redirect_to_checkout'] ) ) {
		...
		$url = add_query_arg( array('redirect_to_checkout'=>false), $url );
		wp_redirect( $url );
		exit;		
	}	
});