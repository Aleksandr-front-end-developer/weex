	        ...
	        <div class="mkdf-post-title-area <?php if ( ! has_post_thumbnail() ) { echo esc_attr( 'mkdf-no-thumbnail' ); } ?>">
		        <div class="mkdf-post-title-area-inner">
			        ...
			        if ( ! empty( $mkdf_my_excerpt ) ) { ?>
				        <p itemprop="description" class="mkdf-post-excerpt"><?php echo wp_trim_words( $mkdf_my_excerpt, 30 ); ?></p>
			        <?php } ?>
		        </div>
	        </div>
        </div>
...
<?php endwhile; ?>
<?php else: ?>
	<p class="mkdf-blog-no-posts"><?php esc_html_e( 'No posts were found.', 'biagiotti' ); ?></p>
<?php endif; ?>