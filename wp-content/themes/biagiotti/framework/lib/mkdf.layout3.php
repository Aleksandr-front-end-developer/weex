...
/*
   Class: BiagiottiMikadoClassMultipleImages
   A class that initializes Mikado Multiple Images
*/

class BiagiottiMikadoClassMultipleImages implements iBiagiottiMikadoInterfaceRender {
	private $name;
	private $label;
	private $description;
	...
	function __construct( $name, $label = "", $description = "" ) {
		...
		$biagiotti_mikado_global_Framework->mkdMetaBoxes->addOption( $this->name, "" );
	}
					...
					<div class="row">
						<div class="col-lg-12">
							...
							<div class="mkdf-gallery-uploader">
								<a class="mkdf-gallery-upload-btn btn btn-sm btn-primary" href="javascript:void(0)"><?php esc_html_e( 'Upload', 'biagiotti' ); ?></a>
								<a class="mkdf-gallery-clear-btn btn btn-sm btn-default pull-right" href="javascript:void(0)"><?php esc_html_e( 'Remove All', 'biagiotti' ); ?></a>
							</div>

							<?php wp_nonce_field( 'mkdf_gallery_upload_get_images_' . esc_attr( $this->name ), 'mkdf_gallery_upload_get_images_' . esc_attr( $this->name ) ); ?>
						</div>
					</div>
				</div>
			</div>
...
class BiagiottiMikadoClassTwitterFramework implements iBiagiottiMikadoInterfaceRender {
	public function render( $factory ) {
		$twitterApi = BiagiottiTwitterApi::getInstance();
		$message    = '';
		...
		if ( ! empty( $_GET['oauth_token'] ) && ! empty( $_GET['oauth_verifier'] ) ) {
			...
			if ( $responseObj->status ) {
				$message = esc_html__( 'You have successfully connected with your Twitter account. If you have any issues fetching data from Twitter try reconnecting.', 'biagiotti' );
			} else {
				$message = $responseObj->message;
			}
		}
		
		$buttonText = $twitterApi->hasUserConnected() ? esc_html__( 'Re-connect with Twitter', 'biagiotti' ) : esc_html__( 'Connect with Twitter', 'biagiotti' );
		?>
		<?php if ( $message !== '' ) { ?>
		...
		<div class="mkdf-page-form-section" id="mkdf_enable_social_share">
			<div class="mkdf-field-desc">
				<h4><?php esc_html_e( 'Connect with Twitter', 'biagiotti' ); ?></h4>
				<p><?php esc_html_e( 'Connecting with Twitter will enable you to show your latest tweets on your site', 'biagiotti' ); ?></p>
			</div>
			<div class="mkdf-section-content">
				<div class="container-fluid">
					<div class="row">
						<div class="col-lg-12">
							...
							<?php wp_nonce_field( 'mkdf_twitter_connect_nonce', 'mkdf_twitter_connect_nonce' ); ?>
						</div>
					</div>
				</div>
			</div>
...
class BiagiottiMikadoClassInstagramFramework implements iBiagiottiMikadoInterfaceRender {
    public function render( $factory ) {
	    ...
	    if ( ! empty( $_GET['status'] ) && ! empty( $_GET['instagram'] ) ) {
		    //update code option so we can use it later
			if( $_GET['status'] == 'success' ){
				$instagram_api->set_connection_type('instagram');
				$instagram_api->instagram_store_codes();
		        $message = esc_html__( 'You have successfully connected with your Instagram Personal account.', 'biagiotti' );
			}else{
	    ...
	    if ( ! empty( $_GET['access_token'] ) && ! empty( $_GET['facebook'] ) ) {
		    //update code option so we can use it later
		    $instagram_api->set_connection_type('facebook');
		    $instagram_api->facebook_store_token();
		    $message = esc_html__( 'You have successfully connected with your Instagram Business account.', 'biagiotti' );
	    }
	    ...
	    if ( ! empty( $_GET['disconnect'] ) ) {
		    ...
		    $message = esc_html__( 'You have have been disconnected from all Instagram accounts.', 'biagiotti' );
	    ...
	    <div class="mkdf-page-form-section" id="mkdf_enable_social_share">
		    <div class="mkdf-field-desc">
			    <h4><?php esc_html_e( 'Connect with Instagram', 'biagiotti' ); ?></h4>
			    <p><?php esc_html_e( 'Connecting with Instagram will enable you to show your latest photos on your site', 'biagiotti' ); ?></p>
		    </div>
		    <div class="mkdf-section-content">
			    <div class="container-fluid">
				    <?php
				    ...
				    if ($instagram_user_id) { ?>
					    <div class="row">
						    <div class="col-lg-12">
							    <p><?php echo esc_html__( 'You are currently connected to Instagram ID: ', 'biagiotti' ); echo esc_attr($instagram_user_id) ?></p>
						    </div>
					    </div>
				    <?php } ?>
				    <div class="row">
					    <?php if ( ! empty( $_GET['disconnect'] ) ) { ?>
						    <div class="col-lg-4">
							    <a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->reload_url() ); ?>"><?php echo esc_html__( 'Reload Page', 'biagiotti' ); ?></a>
						    </div>
					    <?php } else if ( empty( $connection_type ) ) { ?>
						    <div class="col-lg-4">
							    <a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->instagram_request_code() ); ?>"><?php echo esc_html__( 'Connect with Instagram Personal account', 'biagiotti' ); ?></a>
						    </div>
<!--						    <div class="col-lg-4">-->
<!--							    <a class="btn btn-primary" href="--><?php //echo esc_url( $instagram_api->facebook_request_code() ); ?><!--">--><?php //echo esc_html__( 'Connect with Instagram Business account', 'biagiotti' ); ?><!--</a>-->
<!--						    </div>-->
					    <?php } else { ?>
						    <div class="col-lg-4">
							    <a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->disconnect_url() ); ?>"><?php echo esc_html__( 'Disconnect Instagram account', 'biagiotti' ) ?></a>
						    </div>
					    <?php } ?>
				    </div>
			    </div>
...
class BiagiottiMikadoClassRepeater implements iBiagiottiMikadoInterfaceRender {
	private $label;
	private $description;
	private $name;
	private $fields;
	...
	function __construct( $fields, $name, $label = '', $description = '', $button_text = '', $table_layout = false ) {
		...
		$this->name         = $name;
		$this->num_of_rows  = 1;
		$this->button_text  = ! empty( $button_text ) ? $button_text : esc_html__( 'Add New Item', 'biagiotti' );
			...
			<?php if ( $this->table_layout ) { ?>
				<div class="mkdf-repeater-table-heading">
					<div class="mkdf-repeater-fields-holder">
						<div class="mkdf-repeater-table-cell mkdf-repeater-sort"><?php esc_html_e( 'Order', 'biagiotti' ) ?></div>
						...
						<div class="mkdf-repeater-table-cell mkdf-repeater-remove"><?php esc_html_e( 'Remove', 'biagiotti' ) ?></div>
					</div>
				</div>
			<?php } ?>
																			...
																			<div class="mkdf-repeater-inner-fields-row <?php echo esc_attr( $col_width_inner_class ); ?>">
																				<div class="mkdf-repeater-inner-fields-row-inner">
																					...
																					if ( ! empty( $field_inner['dependency'] ) ) {
																						$dependencyValues = biagiotti_mikado_return_repeater_dependency_options_array(array(
																							'field' 	   => $field,
...
class BiagiottiMikadoClassFieldAddress extends BiagiottiMikadoClassFieldType {
					...
					<div class="row">
						<div class="col-lg-<?php echo esc_attr( $col_width ); ?>">
							...
							if ( empty( $google_maps_api_key ) ) { ?>
								<p class="description"><?php esc_html_e( 'In order for the map functionality to be enabled please input the Google Map API key in the General section of the Biagiotti Options', 'biagiotti' ); ?></p>
							<?php } ?>
							<a class="mkdf-reset-marker mkdf-hide-field" href="#"><?php esc_html_e( 'Reset Marker', 'biagiotti' ); ?></a>
...
class BiagiottiMikadoClassFieldIcon extends BiagiottiMikadoClassFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $repeat = array() ) {
		$class = '';
		...
		$icon_packs        = biagiotti_mikado_icon_collections()->getIconCollectionsEmpty();
		$icons_collections = biagiotti_mikado_icon_collections()->getIconCollectionsKeys();
		?>
			...
			<div class="mkdf-section-content">
				<div class="container-fluid">
					...
					<?php foreach ( $icons_collections as $icons_collection ) { ?>
						<?php
						$icons_param = biagiotti_mikado_icon_collections()->getIconCollectionParamNameByKey( $icons_collection );
						...
						<div class="row mkdf-icon-collection-holder <?php echo esc_attr( $field_class ); ?>" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
							<div class="col-lg-<?php echo esc_attr( $col_width ); ?>">
								<select name="<?php echo esc_attr( $name . '[' . $icons_param . ']' ); ?>" class="<?php echo esc_attr( $select2 ) ?> form-control mkdf-form-element">
									<?php
									$icons       = biagiotti_mikado_icon_collections()->getIconCollection( $icons_collection );
