...
include BIAGIOTTI_MIKADO_FRAMEWORK_ICONS_ROOT_DIR . '/icons-pack-interface.php';

foreach ( glob( BIAGIOTTI_MIKADO_FRAMEWORK_ICONS_ROOT_DIR . '/*/load.php' ) as $module_load ) {
	if ( biagiotti_mikado_is_customizer_item_enabled( $module_load, 'biagiotti_performance_disable_icon_pack_' ) ) {
		include_once $module_load;
	}
}

if ( ! function_exists( 'biagiotti_mikado_get_icon_pack_collections' ) ) {
	function biagiotti_mikado_get_icon_pack_collections() {
		$options = apply_filters( 'biagiotti_mikado_filter_add_icon_pack_into_collection', $options = array() );
		
		return $options;
	}
}

if ( ! function_exists( 'biagiotti_mikado_get_icon_pack_options' ) ) {
	function biagiotti_mikado_get_icon_pack_options() {
		$options = apply_filters( 'biagiotti_mikado_filter_add_icon_pack_into_options', $options = array() );
		
		return $options;
	}
}

if ( ! function_exists( 'biagiotti_mikado_get_icon_pack_social_options' ) ) {
	function biagiotti_mikado_get_icon_pack_social_options() {
		$options = apply_filters( 'biagiotti_mikado_filter_add_icon_pack_into_social_options', $options = array() );
...
/*
 * Class: BiagiottiMikadoClassIconCollections
 */
class BiagiottiMikadoClassIconCollections {
	...
	private function __construct() {
		...
		$this->iconPackCollections   = biagiotti_mikado_get_icon_pack_collections();
		$this->iconPackOptions       = biagiotti_mikado_get_icon_pack_options();
		$this->iconPackSocialOptions = biagiotti_mikado_get_icon_pack_social_options();
	...
	public function getIconsMetaBoxOrOption( $attributes ) {
		...
		foreach ( $icon_collections as $collection_key => $collection_object ) {
			...
			$icon_pack_container = biagiotti_mikado_add_admin_container(
				array(
					'parent'     => $parent,
	...
	public function getVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Icon Pack', 'biagiotti' ),
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'        => 'dropdown',
					'heading'     => esc_html__( 'Icon', 'biagiotti' ),
	...
	public function getIconWidgetParamsArray() {
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'name'    => 'icon_pack',
			'title'   => esc_html__( 'Icon Pack', 'biagiotti' ),
			'options' => $this->getIconPackOptions()
		);
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'    => 'dropdown',
					'title'   => $collection->title . esc_html__( ' Icon', 'biagiotti' ),
	...
	public function getSocialIconWidgetMultipleParamsArray( $count ) {
		...
		for ( $n = 1; $n <= $count; $n ++ ) {
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'biagiotti' ), $n, $collection->title ),
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_' . $n,
				'title' => sprintf( esc_html__( 'Link %s', 'biagiotti' ), $n )
			);
			
			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_' . $n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'biagiotti' ), $n ),
	...
	public function getIconSizesArray() {
		return array(
			esc_html__( 'Tiny', 'biagiotti' )       => 'fa-lg',
			esc_html__( 'Small', 'biagiotti' )      => 'fa-2x',
			esc_html__( 'Medium', 'biagiotti' )     => 'fa-3x',
			esc_html__( 'Large', 'biagiotti' )      => 'fa-4x',
			esc_html__( 'Very Large', 'biagiotti' ) => 'fa-5x'
		);
	}
	...
	public function getIconCollectionIcons( iBiagiottiMikadoInterfaceIconCollection $collection ) {
	...
	public function getIconCollectionsEmpty( $no_empty_key = "" ) {
		$array                  = array();
		$array[ $no_empty_key ] = esc_html__( 'No Icon', 'biagiotti' );
	...
	public function getIconCollectionsVCEmpty() {
		$vc_array                                           = array();
		$vc_array[ esc_html__( 'No Icon', 'biagiotti' ) ] = '';
	...
	public function getElementorParamsArray($control_object, $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false){
		...
		$control_object->add_control(
			'icon_pack', [
				'label' => esc_html__( 'Icon Pack', 'biagiotti' ),
			...
			foreach ($this->iconCollections as $key => $collection) {
				$control_object->add_control(
					$iconCollectionPrefix . $collection->param, [
						'label' => esc_html__( 'Icon', 'biagiotti' ),
						'type' => \Elementor\Controls_Manager::SELECT,
						'options' => array_flip($collection->getIconsArray()),
						'condition' => [
							'icon_pack' => $key,
	...
	public function getElementorIconFromIconPack( $params ){
		...
		if( ! empty( $params['linear_icon'] ) ){
			$params['icon'] = $params['linear_icon'];
		}

		if( ! empty( $params['simple_line_icon'] ) ){
			$params['icon'] = $params['simple_line_icon'];
		}
