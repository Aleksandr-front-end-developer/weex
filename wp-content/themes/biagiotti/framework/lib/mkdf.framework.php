...
/*
   Class: BiagiottiMikadoClassFramework
...
class BiagiottiMikadoClassFramework {
	...
	private function __construct() {
		$this->mkdOptions         = BiagiottiMikadoClassOptions::get_instance();
		$this->mkdMetaBoxes       = BiagiottiMikadoClassMetaBoxes::get_instance();
		$this->mkdTaxonomyOptions = BiagiottiMikadoClassTaxonomyOptions::get_instance();
		$this->mkdUserOptions     = BiagiottiMikadoClassUserOptions::get_instance();
		$this->mkdDashboardOptions = BiagiottiMikadoClassDashboardOptions::get_instance();
	}
	...
	public function setSkin( BiagiottiMikadoClassSkinAbstract $skinObject ) {
		$this->skin = $skinObject;
	}
}

/**
 * Class BiagiottiMikadoClassSkinManager
 *
...
class BiagiottiMikadoClassSkinManager {
	...
	/**
	 * @see BiagiottiMikadoClassSkinManager::setSkin()
	 */
	 ...
	 */
	private function setSkin( $skinName = BIAGIOTTI_MIKADO_PROFILE_SLUG ) {
		if ( $skinName !== '' ) {
...
/**
 * Class BiagiottiMikadoClassSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class BiagiottiMikadoClassSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	public function loadTemplatePart( $template, $params = array() ) {
		...
		if ( $template !== '' ) {
			include( biagiotti_mikado_find_template_path( 'framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php' ) );
		}
	}
	...
	public function setShortcodeJSParams() { ?>
		<script>
			window.mkdSCIcon = '<?php echo biagiotti_mikado_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
			window.mkdSCLabel = '<?php echo esc_html( ucfirst( $this->skinName ) ); ?> <?php esc_html_e( 'Shortcodes', 'biagiotti' ) ?>';
		</script>
	<?php }
...
/*
   Class: BiagiottiMikadoClassOptions
...
class BiagiottiMikadoClassOptions {
	...
	public function getOptionValue( $key ) {
		...
		if ( array_key_exists( $key, $biagiotti_mikado_global_options ) ) {
			return $biagiotti_mikado_global_options[ $key ];
...
/*
   Class: BiagiottiMikadoClassAdminPage
   A class that initializes Mikado Admin Page
*/
class BiagiottiMikadoClassAdminPage implements iBiagiottiMikadoInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $slug;
	public $title;
	...
	function __construct( $slug = "", $title = "", $icon = "" ) {
		$this->layout  = array();
		$this->factory = new BiagiottiMikadoClassFieldFactory();
		$this->slug    = $slug;
		$this->title   = $title;
		$this->icon    = $icon;
	}
	...
	public function renderChild( iBiagiottiMikadoInterfaceRender $child ) {
		$child->render( $this->factory );
	}
}

/*
   Class: BiagiottiMikadoClassMetaBoxes
...
/*
   Class: BiagiottiMikadoClassMetaBox
   A class that initializes Mikado Meta Box
*/
class BiagiottiMikadoClassMetaBox implements iBiagiottiMikadoInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	public $title;
	...
	public function setTitle( $label ) {
		...
		return $biagiotti_mikado_global_Framework->getSkin()->getSkinLabel() . ' ' . $label;
	}
}

/*
   Class: BiagiottiMikadoClassTaxonomyOptions
   A class that initializes BiagiottiMikadoClass Taxonomy Options
*/
class BiagiottiMikadoClassTaxonomyOptions {
	private static $instance;
...
/*
   Class: BiagiottiMikadoClassTaxonomyOption
   A class that initializes BiagiottiMikadoClass Taxonomy Option
*/
class BiagiottiMikadoClassTaxonomyOption implements iBiagiottiMikadoInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new BiagiottiMikadoClassTaxonomyFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: BiagiottiMikadoClassUserOptions
   A class that initializes BiagiottiMikadoClass User Options
*/
class BiagiottiMikadoClassUserOptions {
	private static $instance;
	public $userOptions;
...
/*
   Class: BiagiottiMikadoClassUserOption
   A class that initializes BiagiottiMikadoClass User Option
*/
class BiagiottiMikadoClassUserOption implements iBiagiottiMikadoInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new BiagiottiMikadoClassUserFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: BiagiottiMikadoClassDashboardOptions
   A class that initializes BiagiottiMikadoClass Dashboard Options
*/
class BiagiottiMikadoClassDashboardOptions {
	private static $instance;
...
/*
   Class: BiagiottiMikadoClassDashboardOption
   A class that initializes BiagiottiMikadoClass Dashboard Option
*/
class BiagiottiMikadoClassDashboardOption implements iBiagiottiMikadoInterfaceLayoutNode {
	public $layout;
	private $factory;
	
	function __construct() {
		$this->layout        = array();
		$this->factory       = new BiagiottiMikadoClassDashboardFieldFactory();
	}
...
if ( ! function_exists( 'biagiotti_mikado_init_framework_variable' ) ) {
	function biagiotti_mikado_init_framework_variable() {
		global $biagiotti_mikado_global_Framework;
		
		$biagiotti_mikado_global_Framework = BiagiottiMikadoClassFramework::get_instance();
		$mkdSkinManager                 = new BiagiottiMikadoClassSkinManager();
		$biagiotti_mikado_global_Framework->setSkin( $mkdSkinManager->getSkin() );
	}
	
	add_action( 'biagiotti_mikado_action_before_options_map', 'biagiotti_mikado_init_framework_variable' );
}
?>