...
class BiagiottiMikadoElementorProductInfo extends \Elementor\Widget_Base {
    ...
    public function get_title() {
        return esc_html__( 'Product Info', 'biagiotti' );
    }

    public function get_icon() {
        return 'biagiotti-elementor-custom-icon biagiotti-elementor-product-info';
    }
    ...
    protected function register_controls() {
        ...
        $this->add_control(
            'product_id',
            [
                'label'       => esc_html__( 'Selected Product', 'biagiotti' ),
                'type'        => \Elementor\Controls_Manager::TEXT,
                'description' => esc_html__( 'If you left this field empty then product ID will be of the current page', 'biagiotti' )
            ]
        );
        ...
        $this->add_control(
            'product_info_type',
            [
                'label'   => esc_html__( 'Product Info Type', 'biagiotti' ),
                'type'    => \Elementor\Controls_Manager::SELECT,
                'options' => [
                    'title'          => esc_html__( 'Title', 'biagiotti' ),
                    'featured_image' => esc_html__( 'Featured Image', 'biagiotti' ),
                    'category'       => esc_html__( 'Category', 'biagiotti' ),
                    'excerpt'        => esc_html__( 'Excerpt', 'biagiotti' ),
                    'price'          => esc_html__( 'Price', 'biagiotti' ),
                    'rating'         => esc_html__( 'Rating', 'biagiotti' ),
                    'add_to_cart'    => esc_html__( 'Add To Cart', 'biagiotti' ),
                    'tag'            => esc_html__( 'Tag', 'biagiotti' ),
                    'author'         => esc_html__( 'Author', 'biagiotti' ),
                    'date'           => esc_html__( 'Date', 'biagiotti' ),
                ],
                'default' => 'title'
            ]
        );
        ...
        $this->start_controls_section(
            'product_info_style',
            [
                'label' => esc_html__( 'Product Info Style', 'biagiotti' ),
                'tab'   => \Elementor\Controls_Manager::TAB_CONTENT,
            ]
        );
        ...
        $this->add_control(
            'product_info_color',
            [
                'label'     => esc_html__( 'Product Info Color', 'biagiotti' ),
        ...
        $this->add_control(
            ...
            [
                ...
                'description' => esc_html__( 'Set title tag for product title element', 'biagiotti' ),
                'default'     => 'h2'
            ]
        );
        ...
        $this->add_control(
            'featured_image_size',
            [
                'label'     => esc_html__( 'Featured Image Proportions', 'biagiotti' ),
                'type'      => \Elementor\Controls_Manager::SELECT,
                'options'   => [
        ...
        $this->add_control(
            'category_tag',
            [
                'label'       => esc_html__( 'Category Tag', 'biagiotti' ),
                'type'        => \Elementor\Controls_Manager::SELECT,
                'options'     => biagiotti_mikado_get_title_tag( true ),
                'condition'   => [
                    'product_info_type' => array( 'category' )
                ],
                'description' => esc_html__( 'Set category tag for product category element', 'biagiotti' ),
            ]
        );
        ...
        $this->add_control(
            'add_to_cart_skin',
            [
                'label'       => esc_html__( 'Add To Cart Skin', 'biagiotti' ),
                'type'        => \Elementor\Controls_Manager::SELECT,
                'options'     => [
                    ''      => esc_html__( 'Default', 'biagiotti' ),
    ...
    public function render() {
		...
		$params['product_id'] = apply_filters( 'wpml_object_id', $params['product_id'], get_post_type( $params['product_id'] ) );
	...
	public function getItemTitleHtml( $params ) {
		...
		if ( ! empty( $title ) ) {
			$html = '<' . esc_attr( $title_tag ) . ' itemprop="name" class="mkdf-pi-title entry-title" ' . biagiotti_mikado_get_inline_style( $product_info_style ) . '>';
		...
		if ( ! empty( $categories ) ) {
			...
			foreach ( $categories as $cat ) {
				if ( ! empty( $params['category_tag'] ) ) {
					$html .= '<' . esc_attr( $params['category_tag'] ) . ' ' . biagiotti_mikado_get_inline_style( $product_info_style ) . '>';
					...
					$html .= '<a itemprop="url" class="mkdf-pi-category-item" href="' . esc_url( get_term_link( $cat->term_id ) ) . '" ' . biagiotti_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $cat->name ) . '</a>';
				}
			}
			$html .= '</div>';
		}
	...
	public function getItemExcerptHtml( $params ) {
		...
		if ( ! empty( $excerpt ) ) {
			$html = '<div class="mkdf-pi-excerpt"><p itemprop="description" class="mkdf-pi-excerpt-item" ' . biagiotti_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $excerpt ) . '</p></div>';
		}
	...
	public function getItemPriceHtml( $params ) {
		...
		if ( $price_html = $product->get_price_html() ) {
			$html = '<div class="mkdf-pi-price" ' . biagiotti_mikado_get_inline_style( $product_info_style ) . '>' . $price_html . '</div>';
		}
	...
	public function getItemRatingHtml( $params ) {
		...
		if ( get_option( 'woocommerce_enable_review_rating' ) !== 'no' ) {
			...
			$html = '<div class="mkdf-pi-rating" title="' . sprintf( esc_attr__( "Rated %s out of 5", "biagiotti" ), $average ) . '" ' . biagiotti_mikado_get_inline_style( $product_info_style ) . '><span style="width:' . ( ( $average / 5 ) * 100 ) . '%"></span></div>';
		}
	...
	public function getItemAddToCartHtml( $params ) {
		...
		if ( ! $product->is_in_stock() ) {
			$button_classes = 'button ajax_add_to_cart mkdf-button mkdf-out-of-stock';
		...
		if ( ! empty( $params['add_to_cart_skin'] ) ) {
			$button_classes .= ' mkdf-' . $params['add_to_cart_skin'] . '-skin';
		}

		$html = '<div class="mkdf-pi-add-to-cart">';
		$html .= apply_filters( 'biagiotti_mikado_filter_product_info_add_to_cart_link',
	...
	public function getItemTagHtml( $params ) {
		...
		if ( ! empty( $tags ) ) {
			...
			foreach ( $tags as $tag ) {
				$html .= '<a itemprop="url" class="mkdf-pi-tag-item" href="' . esc_url( get_term_link( $tag->term_id ) ) . '" ' . biagiotti_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $tag->name ) . '</a>';
			}
			$html .= '</div>';
		}
	...
	public function getItemAuthorHtml( $params ) {
		...
		if ( ! empty( $author ) ) {
			$html = '<div class="mkdf-pi-author" ' . biagiotti_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $author ) . '</div>';
		}
	...
	public function getItemDateHtml( $params ) {
		...
		if ( ! empty( $date ) ) {
			$html = '<div class="mkdf-pi-date" ' . biagiotti_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $date ) . '</div>';
		}
	...
	public function productIdAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				$data = array();
				$data['value'] = $value['id'];
				$data['label'] = esc_html__( 'Id', 'biagiotti' ) . ': ' . $value['id'] . ( ( strlen( $value['title'] ) > 0 ) ? ' - ' . esc_html__( 'Title', 'biagiotti' ) . ': ' . $value['title'] : '' );
				$results[] = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( ! is_wp_error( $product ) ) {
				...
				if ( ! empty( $product_title ) ) {
					$product_title_display = ' - ' . esc_html__( 'Title', 'biagiotti' ) . ': ' . $product_title;
				}

				$product_id_display = esc_html__( 'Id', 'biagiotti' ) . ': ' . $product_id;
...
\Elementor\Plugin::instance()->widgets_manager->register_widget_type( new BiagiottiMikadoElementorProductInfo() );